...
void writeTree(CompilerInfo* pci, TreeNode* node, int sh = 0) {
    int i, NSH = 3;
    for (i = 0; i < sh; i++) {
        fprintf(pci->out_file.file, " ");
    }

    fprintf(pci->out_file.file, "[%s]", NodeKindStr[node->node_kind]);

    if (node->node_kind == OPER_NODE) {
        fprintf(pci->out_file.file, "[%s]", TokenTypeStr[node->oper]);
    } else if (node->node_kind == NUM_NODE) {
        fprintf(pci->out_file.file, "[%d]", node->num);
    } else if (node->node_kind == ID_NODE || node->node_kind == READ_NODE || node->node_kind == ASSIGN_NODE) {
        fprintf(pci->out_file.file, "[%s]", node->id);
    }

    if (node->expr_data_type != VOID) {
        fprintf(pci->out_file.file, "[%s]", ExprDataTypeStr[node->expr_data_type]);
    }
    ...
    for (i = 0; i < MAX_CHILDREN; i++) {
        if (node->child[i]) {
            writeTree(pci, node->child[i], sh + NSH);
        }
    }

    if (node->sibling) {
        writeTree(pci, node->sibling, sh);
    }
}
